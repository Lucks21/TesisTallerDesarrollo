...
$d = new Database;
// devuelve el libro
$sql = " EXECUTE sp_WEB_reserva_accesorio $numero, $volumen, $parte, $suplemento, '$campus', '$formato', 'reserva'" ;
$d->execute($sql);
...
<tr><td>ADVERTENCIA: Antes de realizar una reserva, debe considerar que de no retirar el libro en un
		  ...
		  <font color="red">suspenderá por cinco días hábiles</font></strong>.
<tr><td>&nbsp;</td></tr>
</table>
...
<?php 
	$total = 0;
	while ($row = $d->get_row()) { 
		$num = $row['nro_registro_principal_existe'];
		$total++; ?>    
      <tr class='tr-color-hi'><td colspan='2' class='texto-tabla'>Título Existencia Principal a Reservar</td></tr>
      <tr> 
      ...
      <tr> 
        ...
        <td width="88%"><?php echo $row['nro_registro_principal_existe'];  ?></td>
      </tr>
      ...
      <tr> 
        ...
        <td width="88%"><?php echo $row['s_titulo']; ?></td>
            ...
            <tr> 
      <td width="22%" align="right" class='texto-bold'>Formato a reservar : </td>
	    <td width="88%"><?php echo $row['nombre_tb_format']; ?></td>
      </tr>
	  <?php 
	  
	  // devuelve el accesorio
	  $sql = " EXECUTE sp_WEB_reserva_accesorio $numero, $volumen, $parte, $suplemento, '$campus', '$formato', 'accesorios'" ;
	  $d->execute($sql);
	  while ($row=$d->get_row()) { 
		$num = $row['nro_registro_existe'];
		$url = "item_reserva_principal.php?numero=".$row['nro_control']."&volumen=".$row['nro_volumen_existe'];
      ...
      <td width="88%">
		  <div class='texto-small-hi'>Usted ha seleccionado una existencia 
		    clasificada como accesorio.
		    </div>
      </td></tr>	
      <?php  } ?>
    </table>
	...
	<table border="0" class='texto-small' width='100%'>
      ...
      <tr><td colspan='2'>Si la biblioteca donde retirará el ejemplar 
